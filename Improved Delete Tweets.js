var authorization = "Bearer AAAAAAAA"; // replace with your authorization value
var ua = navigator.userAgentData.brands.map(brand => `"${brand.brand}";v="${brand.version}"`).join(', ');
var client_tid = ""; // replace with your X-Client-Transaction-Id value
var client_uuid = ""; // replace with your X-Client-Uuid value
...
var username = "House0fCarter" // replace with your username
...
// Define readFile before using it
function readFile(file) {
    const reader = new FileReader();
    reader.onload = function(event) {
        const content = event.target.result;
        
        // Remove the prefix and trailing semicolon from the tweets.js file
        let jsonPart = content.replace(/^window\.YTD\.tweets\.part0\s*=\s*/, '').trim();
        ...
        try {
            ...
            if (Array.isArray(data)) {
                ...
                console.log("JSON data loaded into global variable:", data);
            } else {
                console.error("Parsed data is not an array. Check the file structure.");
            }
        } catch (e) {
...
// Define parseTweetsFromArchive before using it
function parseTweetsFromArchive(data) {
    try {
        if (!Array.isArray(data)) {
            console.error("parseTweetsFromArchive expects an array, but got:", data);
            return [];
        }
        ...
        data.forEach(item => {
            if (item.tweet && item.tweet.id_str) {
                ...
                const startsWithRT = item.tweet.full_text && item.tweet.full_text.startsWith('RT ');
                ...
                if (!isInReplyToExcludedUser &&
                    ((delete_options["unretweet"] === false && startsWithRT === true) ||
        ...
        console.error("Error parsing JSON in parseTweetsFromArchive:", error);
        return [];
    }
}
...
// Once archive is confirmed or if from_archive is false
if (delete_options["from_archive"] == true) {
